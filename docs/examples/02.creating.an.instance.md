...
The `zitilib/models/examples` package contains 2 different models, `tiny` and `smartrouting`. Both of these examples effectively do the same thing at different scales. They share all of the same behavioral and instance model components, and differ only in how they're constructed structurally: the `tiny` model deploys all of the components of the model on a single host in a single region, and the `smartrouting` model uses 4 regions and 12 hosts, spreading the same workload over a much larger deployment.

When we start looking at the programming framework provided by `fablab`, we'll use these models to illustrate concepts about how `fablab` enables scalable, extensible operational coding paradigms.
...
There will be quite a lot of output from `terraform`, as it walks the created instance through installation and update of the latest packages from the operating system repository. By default the model expresses a `t2.micro` instance, which also takes a decent amount of time (as of this writing) to execute.

Ultimately, `terraform` should get to a point where it's completed, and `fablab` waits for the host to restart:
