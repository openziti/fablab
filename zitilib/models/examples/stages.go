...
func (self *stageFactory) addOperationStages(m *model.Model) error {
	...
	m.AddOperatingStage(zitilib_5_operation.Metrics(phase.GetCloser()))
...
func (_ *stageFactory) addListenerStages(m *model.Model) error {
	...
	for _, host := range hosts {
		m.AddOperatingStage(zitilib_5_operation.LoopListener(host, nil, "tcp:0.0.0.0:8171"))
	}
...
func (_ *stageFactory) dialers(m *model.Model, phase fablib_5_operation.Phase) error {
	...
	for _, host := range hosts {
		m.AddOperatingStage(zitilib_5_operation.LoopDialer(host, "10-ambient.loop2.yml", endpoint, phase.AddJoiner()))
	}
