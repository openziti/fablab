...
func (i *iperf) Operate(run model.Run) error {
	...
	for _, host := range clientHosts {
		...
		if i.overlay == false {
			iperfCmdU := fmt.Sprintf("iperf3 -c %s -p 7001 -t %d -P 1 -b 5M --json", serverHost.PublicIp, i.seconds)
			...
			iperfCmdO := fmt.Sprintf("iperf3 -c %s -p 7001 -t %d -P 1 -b 5M --json", "iperf.service", i.seconds)
