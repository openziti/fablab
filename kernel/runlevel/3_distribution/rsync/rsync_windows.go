...
func rsync(sourcePath, targetPath string) error {
	...
	if !strings.Contains(zitilab_bootstrap.RsyncCommand(), "cygwin") {
		logrus.Warn("on Windows it is highly suggested that Cygwin's rsync binary is used")
	}

	if !strings.Contains(zitilab_bootstrap.SshCommand(), "cygwin") {
	...
	rsync := internal.NewProcess(zitilab_bootstrap.RsyncCommand(), "-avz", "-e", zitilab_bootstrap.SshCommand()+` -o StrictHostKeyChecking=no`, "--delete", ".", targetPath)
